        ...
        fetch('/api/settings')
            ...
            .then(data => {
                if (data.success && data.data) {
                    ...
                    setSettings({
                        targetAmount: Number(data.data.targetAmount) || 0,
                        raisedAmount: Number(data.data.raisedAmount) || 0
                    })
                }
            })
                    ...
                    <div className="min-h-[380px]">
                        ...
                        {step === 1 && (
                            <div className="space-y-6 animate-fadeIn">
                                ...
                                {/* Progress Bar showing Current Amount in Account */}
                                {settings.targetAmount > 0 && (
                                    <div className="bg-slate-50/40 dark:bg-slate-900/20 border border-border/50 rounded-2xl p-5 space-y-3">
                                        <div className="flex justify-between items-center text-sm font-bold">
                                            <span className="text-slate-500">Fundraising Progress</span>
                                            <span className="text-primary font-bold">
                                                ₹{settings.raisedAmount.toLocaleString('en-IN')} / ₹{settings.targetAmount.toLocaleString('en-IN')}
                                            </span>
                                        </div>
                                        <div className="w-full bg-slate-100 dark:bg-slate-800 h-3 rounded-full overflow-hidden">
                                            <div 
                                                className="h-full bg-gradient-to-r from-violet-500 to-emerald-600 rounded-full transition-all duration-1000"
                                                style={{ width: `${Math.min(100, Math.round((settings.raisedAmount / settings.targetAmount) * 100))}%` }}
                                            />
                                        </div>
                                        <div className="text-xs text-muted-foreground flex justify-between font-medium">
                                            <span>{Math.min(100, Math.round((settings.raisedAmount / settings.targetAmount) * 100))}% Completed</span>
                                            <span>Current raised amount in account</span>
                                        </div>
                                    </div>
                                )}
